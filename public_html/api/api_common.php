...
$viewquery = <<< EOSQL
    CREATE TEMPORARY VIEW disp_temp AS
        SELECT country.iso3 AS code, country.name AS name,
            ...
            gdrs.r_MtC/(SELECT SUM(r_MtC) FROM gdrs WHERE year = core.year) AS gdrs_r_frac,
            ...
            gdrs.c_blnUSDMER/(SELECT SUM(c_blnUSDMER) FROM gdrs WHERE year = core.year) AS gdrs_c_frac,
