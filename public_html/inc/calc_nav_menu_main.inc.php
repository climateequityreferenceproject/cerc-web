<div id="nav">
    <ul class="group">
        <li><a href="http://gdrights.org/Calculator-about">About the Calculator</a></li> <!-- and Scorecard -->
        <li><a href="http://gdrights.org/scorecard_dev/">Climate Equity Pledge Scorecard</a></li>
        <li><a href="http://gdrights.org/about">About Greenhouse Development Rights</a></li>
        <li><a href="http://www.ecoequity.org">EcoEquity </a></li>
        <li class="last"><a href="http://www.sei-international.org">SEI</a></li>
    </ul>
</div><!-- end #nav -->